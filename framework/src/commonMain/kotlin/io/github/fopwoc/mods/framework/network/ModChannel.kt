 ...
 *
 * Declare every message once, as a property of the channel object, so both sides number them in the
 * same order. Create the channel during mod initialization; attach handlers where the receiving
 * side is set up. Declaring is side-neutral, so a dedicated server can send a clientbound message
 * without ever installing its handler.
 *
 * ```kotlin
...
internal constructor(
     ...
     */
    val isAvailableOnServer: Boolean
        get() = backend.isAvailableOnServer(this)
    ...
    fun <P : Any> serverbound(codec: MessageCodec<P>): Serverbound<P> =
        Serverbound(messages.size, codec).also(messages::add)

    /** Server → client messages. */
    fun <P : Any> clientbound(codec: MessageCodec<P>): Clientbound<P> =
        Clientbound(messages.size, codec).also(messages::add)

    /**
     * Backends pass every received frame; [sender] is the sending player on the server, null on the
     * client.
     */
    ...
    sealed class Message<P : Any>(private val index: Int, protected val codec: MessageCodec<P>) {
        ...
        protected fun frame(protocolVersion: Int, payload: P): ByteArray =
            MessageWriter()
                .byte(index)
                .int(protocolVersion)
                .also { codec.encode(it, payload) }
                .toByteArray()
    }

    inner class Serverbound<P : Any> internal constructor(index: Int, codec: MessageCodec<P>) :
        Message<P>(index, codec) {
        ...
        fun send(payload: P) =
            backend.sendToServer(this@ModChannel, frame(protocolVersion, payload))
    ...
    inner class Clientbound<P : Any> internal constructor(index: Int, codec: MessageCodec<P>) :
        Message<P>(index, codec) {
