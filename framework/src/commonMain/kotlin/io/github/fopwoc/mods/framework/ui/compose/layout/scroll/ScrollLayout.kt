...
internal fun resolveScrollMetrics(
    ...
    val gutterSize =
        if (contentMainAxisSize > scrollArea.mainAxisSize(axis)) {
            SCROLLBAR_GUTTER_WIDTH.coerceAtMost(scrollArea.crossAxisSize(axis))
        } else {
            0
        }
    val viewportBounds =
...
internal fun resolveScrollThumbBounds(metrics: ScrollMetrics): Rect? {
    ...
    if (
        metrics.maxValue <= 0 ||
            metrics.viewportBounds.width <= 0 ||
            metrics.viewportBounds.height <= 0
    ) {
        return null
    }
    ...
    val thumbLength =
        max(
                16,
                viewportMainAxisSize * viewportMainAxisSize /
                    metrics.contentMainAxisSize.coerceAtLeast(1),
            )
            .coerceAtMost(viewportMainAxisSize)
    val thumbTravel = (trackBounds.mainAxisSize(metrics.axis) - thumbLength).coerceAtLeast(0)
    val thumbStart =
        trackBounds.mainAxisStart(metrics.axis) +
...
private fun resolveScrollTrackBounds(
    ...
    return when (axis) {
        StackAxis.VERTICAL ->
            Rect(
                x = viewportBounds.x + viewportBounds.width + SCROLLBAR_TRACK_INSET,
                y = scrollArea.y,
                width = SCROLLBAR_TRACK_WIDTH.coerceAtMost(gutterSize),
                height = scrollArea.height,
            )
        StackAxis.HORIZONTAL ->
            Rect(
                x = scrollArea.x,
                y = viewportBounds.y + viewportBounds.height + SCROLLBAR_TRACK_INSET,
                width = scrollArea.width,
                height = SCROLLBAR_TRACK_WIDTH.coerceAtMost(gutterSize),
            )
    }
}
