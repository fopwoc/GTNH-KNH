...
internal class ComposeGuiRuntime(
    ...
    private val errorHandler: ComposeRuntimeErrorHandler = ComposeRuntimeErrorHandler { _, _ -> },
    ...
    fun start(rootNode: RootNode, content: @Composable () -> Unit) {
        ...
        try {
            ...
            val scope =
                CoroutineScope(
                    SupervisorJob() + ComposeMainDispatcher + frameClock + exceptionHandler
                )
     ...
     * logged when they happen, but a dead recomposer would otherwise leave the UI silently frozen,
     * so hosts call this from their render path to turn the failure into a regular crash.
     */
        ...
        do {
            ...
            drainedMainDispatcherTasks =
                ComposeMainDispatcherBridge.pump(maxComposeTaskExecutionsPerPump) {
                    "ComposeGuiRuntime.pump() exceeded $maxComposeTaskExecutionsPerPump " +
                        "compose task executions without reaching an idle state"
                }
            flushedSnapshotNotifications = flushSnapshotNotifications()
        } while (
            snapshotNotificationsPending ||
                drainedMainDispatcherTasks ||
                flushedSnapshotNotifications
        )
    }
    ...
    private fun recordFailure(message: String, throwable: Throwable) {
        errorHandler.report(message, throwable)
