...
internal object ComposeMainDispatcherBridge {
        ...
        synchronized(lock) {
            ...
            when {
                ...
                installedThread === currentThread -> {
                    ...
                    error(
                        "ComposeMainDispatcher is already installed for thread ${installedThread.name} " +
                            "and cannot also bind to ${currentThread.name}"
                    )
                }
            }
        }
    ...
    fun releaseForCurrentThread() {
        ...
        synchronized(lock) {
            ...
            check(state.mainThread === currentThread) {
                "ComposeMainDispatcher is bound to thread ${state.mainThread?.name} " +
                    "and must be released from it, not from ${currentThread.name}"
            }
    ...
    fun pump(): Boolean =
        pump(Int.MAX_VALUE) {
            "ComposeMainDispatcherBridge.pump() exceeded ${Int.MAX_VALUE} task executions " +
                "without reaching an idle state"
        }
