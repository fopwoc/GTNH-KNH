    ...
    fun defaultsApplyWithoutAProviderAndOverridesReachThemedComponents() = runBlocking {
        ...
        try {
            composition.setContent {
                ...
                MinecraftTheme(
                    colors = MinecraftTheme.colors.copy(title = red, surfaceBorder = blue)
                ) {
                    Text("themed", style = MinecraftTheme.typography.sectionTitle)
                    Section(title = "s") {
                        Text("inner", style = MinecraftTheme.typography.sectionTitle)
                    }
                }
            }
            ...
            val texts =
                root.descendants().filterIsInstance<TextNode>().associateBy { it.text.plainText }
            ...
            val bordered =
                root.descendants().filterIsInstance<BoxNode>().mapNotNull {
                    it.modifier.borderColor
                }
