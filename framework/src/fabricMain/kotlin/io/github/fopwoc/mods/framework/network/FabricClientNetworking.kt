...
internal object FabricClientNetworking {
    fun register(type: CustomPacketPayload.Type<FramePayload>, channel: ModChannel) {
        ClientPlayNetworking.registerGlobalReceiver(type) { payload, _ -> channel.receive(payload.frame, null) }
    }

    fun canSend(type: CustomPacketPayload.Type<FramePayload>): Boolean = ClientPlayNetworking.canSend(type)
