...
internal object GlassSurfaces {
     ...
     * and a bright ring where the shell crosses eye height — also the layer to build when placing
     * the sphere block by block.
     */
    private fun sphereGrid(
        ...
        if (ringRadius > 0.0) {
            ...
            for (index in 0 until segments) {
                ...
                GL11.glVertex3d(
                    originX + cos(theta) * ringRadius,
                    eyeY,
                    originZ + sin(theta) * ringRadius,
                )
            }
            GL11.glEnd()
        }
    ...
    private fun sphereVertex(
        ...
        // Nearer shell brighter than the far side, so an off-centre viewer feels which wall is
        // close.
        val dx = x
        val dy = y - eyeY
        val dz = z
