...
class LiveJsonConfig<T> @PublishedApi internal constructor(
    ...
    fun load(): T = synchronized(this) {
        val loadedValue = readCurrentFileOrNull() ?: defaultNormalizedValue()
        applyLoadedValue(loadedValue, countAsRevision = true)
    }

    fun current(): T = currentValue ?: synchronized(this) {
        currentValue ?: load()
    }

    fun refreshIfChanged(): Boolean = synchronized(this) {
        ...
        val nextValue = if (stamp.exists) {
            readCurrentFileOrNull() ?: run {
