...
class PixelCanvas(val width: Int, val height: Int, val tileSize: Int = 16) {
  ...
  fun fill(region: PixelRegion, argb: Int) {
    ...
    synchronized(lock) {
      repeat(region.height) { row ->
        pixels.fill(argb, (region.top + row) * width + region.left, (region.top + row) * width + region.right)
      }
      markDirty(region)
    }
  }
  ...
  fun snapshotDirty(): PixelSnapshot? = synchronized(lock) {
    ...
    val region = PixelRegion(left, top, minOf(tileSize, width - left), minOf(tileSize, height - top))
