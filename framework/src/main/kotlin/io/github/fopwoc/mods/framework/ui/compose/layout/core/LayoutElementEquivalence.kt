...
internal fun LayoutElement.isLayoutEquivalentTo(other: LayoutElement): Boolean {
  return toLayoutShape() == other.toLayoutShape() &&
      childElementsForLayoutEquivalence()
          .areLayoutEquivalentTo(other.childElementsForLayoutEquivalence())
}
...
private fun LayoutElement.childElementsForLayoutEquivalence(): List<LayoutElement> =
    when (this) {
...
private fun List<LayoutElement>.areLayoutEquivalentToComposeNodes(
    other: List<ComposeTreeNode>
): Boolean {
  return size == other.size &&
      indices.all { index ->
        this[index].isLayoutEquivalentTo(other[index])
...
private fun List<ComposeTreeNode>.areComposeLayoutEquivalentTo(
    other: List<ComposeTreeNode>
): Boolean {
  return size == other.size &&
      indices.all { index ->
