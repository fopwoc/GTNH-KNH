...
internal object LayoutEngine {
  ...
  private fun measure(
    ...
    return when (element) {
      ...
      is LayoutElement.ScrollableColumn ->
          measureScrollableColumn(element, metrics, clampedMaxWidth, clampedMaxHeight)
      is LayoutElement.ScrollableRow ->
          measureScrollableRow(element, metrics, clampedMaxWidth, clampedMaxHeight)
      ...
      is LayoutElement.Checkbox ->
          measureCheckbox(element, metrics, clampedMaxWidth, clampedMaxHeight)
      ...
      is LayoutElement.SelectableList ->
          measureSelectableList(element, metrics, clampedMaxWidth, clampedMaxHeight)
  ...
  private fun measure(
    ...
    return when (node) {
      ...
      is SelectableListNode ->
          measureSelectableList(node, metrics, clampedMaxWidth, clampedMaxHeight)
  ...
  private fun measureBoxNode(
    ...
    val measuredChildren =
        node.children.zip(initiallyMeasuredChildren).map { (child, initialMeasurement) ->
    ...
    val naturalSize =
        Size(
            width =
                kotlin.math.max(
                    98,
                    metrics.textWidth(node.text.formattedString) + 20 + padding.horizontalValue,
                ),
    ...
    val naturalSize =
        Size(
            width =
                kotlin.math.max(
                    11,
                    metrics.textWidth(node.label.formattedString) + 13 + padding.horizontalValue,
                ),
            height =
                kotlin.math.max(
                    11,
                    kotlin.math.max(metrics.lineHeight, 11) + padding.verticalValue,
                ),
        )
    return measureLeaf(node, naturalSize.width, naturalSize.height, maxWidth, maxHeight)
  }
  ...
  private fun place(
    ...
    val scrollMetrics =
        when (val element = measured.element) {
