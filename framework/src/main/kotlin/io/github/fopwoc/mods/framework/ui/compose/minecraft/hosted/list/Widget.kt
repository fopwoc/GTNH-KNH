...
internal class HostedSelectableList(
  ...
  fun handleDrag(mouseY: Int): Boolean {
    return when (dragMode) {
      DragMode.SCROLLBAR -> {
        ...
        val desiredTop =
            (mouseY - (dragAnchorMouseY - thumb.y)).coerceIn(trackTop, trackTop + trackHeight)
  ...
  private fun drawScrollbar(tessellator: Tessellator) {
    ...
    tessellator.addVertexWithUV(
        trackLeft.toDouble(),
        (thumb.y + thumb.height - 1).toDouble(),
        0.0,
        0.0,
        1.0,
    )
    tessellator.addVertexWithUV(
        (trackRight - 1).toDouble(),
        (thumb.y + thumb.height - 1).toDouble(),
        0.0,
        1.0,
        1.0,
  ...
  private fun scrollbarThumbBounds(): Rect? {
    ...
    val thumbHeight =
        max(32, viewportHeight * viewportHeight / getContentHeight().coerceAtLeast(1))
