...
internal class HostedSelectableList(
    ...
    fun update(bounds: Rect, items: List<String>, selectedIndex: Int, onSelectedIndexChange: (Int) -> Unit) {
        setDimensions(bounds.width, bounds.height, bounds.y, bounds.y + bounds.height)
    ...
    fun handleClick(mouseX: Int, mouseY: Int): Boolean {
        ...
        val index = getSlotIndexFromScreenCoords(mouseX, mouseY)
        if (index >= 0) {
            selectedIndex = index
    ...
    fun handleDrag(mouseY: Int): Boolean {
        return when (dragMode) {
            DragMode.SCROLLBAR -> {
                ...
                val maxScroll = getMaxScroll().coerceAtLeast(0)
                if (maxScroll <= 0) {
                    return false
                }
                val trackTop = top
    ...
    fun handleWheel(wheelDelta: Int): Boolean {
        if (wheelDelta == 0 || getMaxScroll() <= 0) {
            return false
        }
    ...
    private fun setDimensions(width: Int, height: Int, top: Int, bottom: Int) {
        this.width = width
        this.height = height
        this.top = top
        this.bottom = bottom
    ...
    private fun getSlotIndexFromScreenCoords(mouseX: Int, mouseY: Int): Int {
        val listLeft = left + width / 2 - getListWidth() / 2
        val listRight = left + width / 2 + getListWidth() / 2
        val relativeY = mouseY - top - headerPadding + getAmountScrolled() - 4
        val index = relativeY / slotHeight
        return if (
    ...
    private fun clampAmountScrolled() {
        val maxScroll = getMaxScroll().let { if (it < 0) it / 2 else it }
        val bounded = when {
    ...
    private fun drawOverlayBackground(startY: Int, endY: Int) {
        ...
        tessellator.addVertexWithUV(left.toDouble(), endY.toDouble(), 0.0, 0.0, (endY / textureScale).toDouble())
        tessellator.addVertexWithUV((left + width).toDouble(), endY.toDouble(), 0.0, (width / textureScale).toDouble(), (endY / textureScale).toDouble())
        tessellator.setColorRGBA_I(0x404040, 255)
        tessellator.addVertexWithUV((left + width).toDouble(), startY.toDouble(), 0.0, (width / textureScale).toDouble(), (startY / textureScale).toDouble())
        tessellator.addVertexWithUV(left.toDouble(), startY.toDouble(), 0.0, 0.0, (startY / textureScale).toDouble())
        tessellator.draw()
    }
