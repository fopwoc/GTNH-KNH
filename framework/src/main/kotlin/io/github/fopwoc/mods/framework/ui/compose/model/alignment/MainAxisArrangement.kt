...
internal fun arrangeMainAxis(
    ...
    return when (distribution) {
        ...
        MainAxisDistribution.SPACE_BETWEEN -> arrangeDistributed(
            childSizes,
            leadingSpace = 0.0,
            gapSpace = if (childSizes.size > 1) freeSpace.toDouble() / (childSizes.size - 1).toDouble() else 0.0
        )
        MainAxisDistribution.SPACE_AROUND -> arrangeDistributed(
            childSizes,
            leadingSpace = if (childSizes.isNotEmpty()) freeSpace.toDouble() / childSizes.size.toDouble() / 2.0 else 0.0,
            gapSpace = if (childSizes.isNotEmpty()) freeSpace.toDouble() / childSizes.size.toDouble() else 0.0
        )
        MainAxisDistribution.SPACE_EVENLY -> arrangeDistributed(
            childSizes,
