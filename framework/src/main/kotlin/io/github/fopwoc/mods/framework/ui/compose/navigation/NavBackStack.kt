...
fun <K : NavKey, S : Any> rememberNavBackStack(
    ...
    rememberSaveable(saver = navBackStackSaver(keySaver)) {
      navBackStackOf(*initialKeys)
    }

internal fun <K : NavKey, S : Any> navBackStackSaver(
    keySaver: Saver<K, S>
): Saver<NavBackStack<K>, Any> =
    Saver(
        save = {
          val savedEntries =
              buildList<List<Any>>() {
          ...
          val restoredEntries =
              buildList<NavEntry<K>> {
                savedEntries.forEach { savedEntry ->
                  ...
                  val restoredKey =
                      keySaver.restore(values.getOrNull(1) as? S ?: return@Saver null)
                          ?: return@Saver null
