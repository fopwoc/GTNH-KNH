...
class NeoForgeNetworkBackend : NetworkBackend {
    ...
    override fun register(channel: ModChannel) {
        ...
        types[channel] = type
        val container =
            ModList.get().getModContainerById(channel.namespace).orElseThrow {
                IllegalStateException(
                    "Channel ${channel.id} needs a mod with id ${channel.namespace}"
                )
            }
        container.eventBus?.addListener(RegisterPayloadHandlersEvent::class.java) { event ->
            val handler =
                IPayloadHandler<FramePayload> { payload, context ->
                    val sender =
                        if (context.flow() == PacketFlow.SERVERBOUND)
                            context.player().toGamePlayer()
                        else null
    ...
    override fun sendToServer(channel: ModChannel, frame: ByteArray) =
        NeoForgeClientNetworking.send(type(channel), frame)

    override fun sendToPlayer(channel: ModChannel, player: GamePlayer, frame: ByteArray) {
        val entity =
            ServerLifecycleHooks.getCurrentServer()?.playerList?.getPlayer(player.id) ?: return
        val type = type(channel)
        if (entity.connection.hasChannel(type))
            PacketDistributor.sendToPlayer(entity, FramePayload(type, frame))
    }

    override fun isAvailableOnServer(channel: ModChannel): Boolean =
        NeoForgeClientNetworking.canSend(type(channel))
