...
internal fun Project.configureIdeProjection(extension: KnhMpExtension, islands: List<KnhMpIsland>) {
    ...
    extension.sourceSets.names().forEach { name ->
        kotlin.sourceSets.maybeCreate(name).apply {
            // Java roots (mixins) are listed as Kotlin roots: kotlinc resolves them and IntelliJ
            // marks them as sources.
            this.kotlin.setSrcDirs(
                kotlinSourceRoots(extension, name) + javaSourceRoot(name) + legacyJavaRoot(name)
            )
    ...
    islands.forEach { island ->
        kotlin.sourceSets.getByName(island.sourceSet).dependencies {
            implementation(island.ideClasspath())
        }
    }
    // Module dependencies become KMP project dependencies so shared API is source-navigable and
    // shared code compiles: module scope -> root shared source sets, target/variant scope ->
    // leaves.
    ...
    val roots =
        extension.sourceSets
            .all()
            .filter { it.parents().isEmpty() && it.name() !in leaves }
            .map { it.name() }
    fun org.jetbrains.kotlin.gradle.plugin.KotlinDependencyHandler.declare(
        dependency: KnhMpDependencyDeclaration
    ) {
        val notation: Any =
            when (dependency) {
        ...
        when (dependency.configuration) {
            KnhMpDependencies.API_CONFIGURATION,
            KnhMpDependencies.BUNDLE_CONFIGURATION -> api(notation)
            "compileOnly" -> compileOnly(notation)
            "runtimeOnly" -> runtimeOnly(notation)
            KnhMpDependencies.TEST_CONFIGURATION,
            KnhMpDependencies.NEOFORGE_CONFIGURATION -> Unit
            else -> implementation(notation)
        }
    }
    extension.common.dependencies
        .resolve()
        .filterNot {
            it is KnhMpDependencyDeclaration.External &&
                it.configuration !in
                    setOf("api", "bundle", "implementation", "compileOnly", "runtimeOnly")
        }
        .forEach { dependency ->
            roots.forEach { root ->
                kotlin.sourceSets.getByName(root).dependencies { declare(dependency) }
            }
        }
            ...
            val scopes =
                listOf(target) +
                    target.minecraftVersions
                        .filter { target.variantSourceSet(it) == leaf }
                        .mapNotNull(target::variantScope)
            scopes
                .flatMap { it.dependencies.resolve() }
                .filterIsInstance<KnhMpDependencyDeclaration.Module>()
                .distinctBy { it.path }
                .forEach { dependency ->
                    kotlin.sourceSets.getByName(leaf).dependencies { declare(dependency) }
                }
        }
    }
    ...
    val carrierJvmTarget =
        extension.targets.all().maxOf { target ->
            target.sourceSets().maxOf {
                extension.sourceSets.effectiveJvmTarget(it, target.bytecodeMinimum)
            }
        }
    ...
    tasks.withType(org.gradle.api.tasks.compile.JavaCompile::class.java).configureEach {
        it.options.release.set(carrierJvmTarget)
    }
    // The facade's own jars are internal (module dependencies, metadata); build/libs holds island
    // jars only.
    tasks.withType(org.gradle.api.tasks.bundling.Jar::class.java).configureEach {
        it.destinationDirectory.set(layout.buildDirectory.dir("facade"))
    }
    ...
    val sharedJvmTarget =
        carriers.getValue("main").maxOfOrNull {
            extension.sourceSets.effectiveJvmTarget(it, MIN_JVM_TARGET)
        }
    ...
    carriers.forEach { (carrier, sourceSets) ->
        ...
        val level =
            if (carrier == "main") minimumApi
                ...
                extension.targets
                    ...
                    .let { target ->
                        target.minecraftVersions
                            .map { extension.effectiveConfiguration(target, it).kotlinApiVersion }
                            .ifEmpty {
                                listOf(
                                    extension.effectiveConfiguration(target, null).kotlinApiVersion
                                )
                            }
                    }
                    .filterNotNull()
        ...
        // depends on commonMain (KotlinSourceSetTreeDependsOnMismatch). The classifier that would
        // put
 ...
 */
private fun Project.configureCommonTests(
    extension: KnhMpExtension,
    kotlin: KotlinMultiplatformExtension,
) {
    // The `ide` test compilation is associated with `main`, which holds exactly the shared source
    // sets, so commonTest sees shared code and nothing platform-specific (KGP forbids dependsOn
    // here).
    val commonTest =
        kotlin.sourceSets.maybeCreate("commonTest").apply {
                    ...
                    .forEach { dependency ->
                        when (dependency) {
                            ...
                            is KnhMpDependencyDeclaration.Module ->
                                implementation(project(dependency.path))
                        }
                    }
            }
        }
...
private fun Project.configurePlatformTests(
    ...
    islands.groupBy(KnhMpIsland::sourceSet).mapTo(linkedSetOf()) { (mainSourceSet, sourceSetIslands)
        ->
...
/**
 * `main` -> source sets shared by every leaf (platform-free); one compilation named after each leaf
 * -> that leaf. Intermediates shared by only some leaves (e.g. a vanilla-only layer under the 26.x
 * loaders) are compiled through those leaves' compilations and get no compilation of their own.
 */
private fun ideCarriers(extension: KnhMpExtension): Map<String, List<String>> {
    ...
    val shared =
        extension.sourceSets
            .names()
            .filter { name -> name !in leaves && closures.all { name in it } }
            .sorted()
    return linkedMapOf("main" to shared) +
        leaves.associateBy { it.removeSuffix("Main") }.mapValues { listOf(it.value) }
}

/**
 * Carrier compilation whose compile classpath resolves a source set for the IDE; intermediates use
 * the first leaf below them.
 */
private fun ideCarrierOf(
    extension: KnhMpExtension,
    carriers: Map<String, List<String>>,
): Map<String, String> {
    val leaves = extension.targets.all().flatMap { it.sourceSets() }
    val direct =
        carriers.flatMap { (carrier, sourceSets) -> sourceSets.map { it to carrier } }.toMap()
    return extension.sourceSets.names().associateWith { name ->
        direct[name]
            ?: leaves.first { name in extension.sourceSets.closure(it) }.removeSuffix("Main")
    }
}
 ...
 * yield JVM artifacts for module dependencies (`framework-ide.jar`), which IntelliJ maps back to
 * the producing source sets; the metadata configurations would yield a metadata jar it cannot map.
 */
...
private fun Project.configureIdeDependencyResolver(
            ...
            artifactResolutionStrategy =
                IdeBinaryDependencyResolver.ArtifactResolutionStrategy.ResolvableConfiguration(
                    configurationSelector = { sourceSet ->
                        ...
                        val compilation =
                            if (carrier == "main") ""
                            else carrier.replaceFirstChar { it.titlecase(Locale.ROOT) }
                        configurations.getByName("ide${compilation}CompileClasspath")
                    }
                )
        )
    // The island export tasks run before KGP resolves IDE dependencies, so the classpath files
    // exist.
    val buildDependencies = islands.map { "$path:${it.ideClasspathTask}" }
    val resolver =
        object : IdeDependencyResolver, IdeDependencyResolver.WithBuildDependencies {
    ...
    IdeMultiplatformImport.instance(this)
        .registerDependencyResolver(
            resolver = resolver,
            constraint =
                IdeMultiplatformImport.SourceSetConstraint {
...
private fun Project.registerIdeVerification(
    ...
    tasks.register("verifyIdeFacade") { task ->
        ...
        task.description =
            "Compiles every IDE main and loader-test compilation and checks the facade source graph."
        task.dependsOn(
        ...
        task.doLast {
            ...
            extension.sourceSets.all().forEach { logicalSourceSet ->
                val actual =
                    kotlin.sourceSets.getByName(logicalSourceSet.name()).dependsOn.mapTo(
                        linkedSetOf()
                    ) {
                        it.name
                    }
    ...
    tasks.register("verifyIdeDependencyModel") { task ->
        task.group = "verification"
        task.description =
            "Checks the source graph and isolated classpaths exported to IntelliJ by KGP."
...
private fun Project.verifyExportedIdeModel(extension: KnhMpExtension, islands: List<KnhMpIsland>) {
    ...
    fun exportedClasspath(name: String): Set<File> =
        entries(name)
            .flatMap { (it["classpath"] as? List<*>).orEmpty() }
            .map { it.toString() }
            .mapTo(linkedSetOf()) { path -> resolveExportedPath(path) }

    fun exportedParents(name: String): Set<String> =
        entries(name)
    ...
    val platformClasses =
        listOf(modernMinecraft, legacyMinecraft, fabric, neoforge, legacyFml, legacyForge)

    val commonClasspath = exportedClasspath("commonMain")
    check(platformClasses.none { commonClasspath.containsClass(it) }) {
        "commonMain exports a Minecraft or loader API"
    }
    ...
    islands.forEach { island ->
        ...
        when (island.target.name) {
            "gtnh" -> {
                check(classpath.containsClass(legacyMinecraft)) {
                    "$label does not export Minecraft 1.7.10"
                }
                check(classpath.containsClass(legacyFml)) { "$label does not export FML 1.7.10" }
                check(classpath.containsClass(legacyForge)) {
                    "$label does not export Forge 1.7.10"
                }
                check(!classpath.containsClass(fabric) && !classpath.containsClass(neoforge)) {
                    "$label exports a modern loader API"
                }
            }
            "fabric" -> {
                ...
                check(
                    !classpath.containsClass(neoforge) &&
                        !classpath.containsClass(legacyFml) &&
                        !classpath.containsClass(legacyForge)
                ) {
            ...
            "neoforge" -> {
                check(
                    classpath.containsClass(modernMinecraft) && classpath.containsClass(neoforge)
                ) {
                    "$label does not export modern Minecraft and NeoForge"
                }
                check(
                    !classpath.containsClass(fabric) &&
                        !classpath.containsClass(legacyFml) &&
                        !classpath.containsClass(legacyForge)
                ) {
                    "$label exports a sibling loader API"
