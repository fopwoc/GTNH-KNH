...
internal fun singleNode(extension: KnhMpExtension, target: KnhMpTarget): List<KnhMpIslandNode> {
    ...
    return listOf(KnhMpIslandNode(version, ":", target.variantSourceSet(version), extension.effectiveConfiguration(target, version)))
}
...
internal fun Project.createIslands(extension: KnhMpExtension): List<KnhMpIsland> = extension.targets.all().flatMap { target ->
        when (target.name) {
            "gtnh" -> listOf(KnhMpGtnhIsland(this, extension, target))
            "neoforge" -> stonecutterIslands(extension, target) { name, nodes -> KnhMpNeoforgeIsland(this, extension, target, name, nodes) }
            "fabric" -> stonecutterIslands(extension, target) { name, nodes -> KnhMpFabricIsland(this, extension, target, name, nodes) }
 ...
 */
internal fun Project.dependencyNode(target: KnhMpTarget, node: KnhMpIslandNode, modulePath: String): Pair<KnhMpIsland, KnhMpIslandNode> {
    val candidates = islandsOf(modulePath).filter { it.target.name == target.name }
    check(candidates.isNotEmpty()) { "$modulePath declares no ${target.name} target, required by $path" }
    return candidates.firstNotNullOfOrNull { island ->
        island.nodes.firstOrNull { it.minecraftVersion == node.minecraftVersion }?.let { island to it }
    } ?: error("$modulePath declares no ${target.name} ${node.minecraftVersion ?: ""} variant, required by $path")
}

/** One resolved module dependency of a node: the producing node and how the consumer sees it. */
internal class KnhMpResolvedModuleDependency(val configuration: String, val island: KnhMpIsland, val node: KnhMpIslandNode)
 ...
 */
internal fun Project.resolvedModuleDependencies(target: KnhMpTarget, node: KnhMpIslandNode): List<KnhMpResolvedModuleDependency> {
    ...
    fun visit(from: Project, fromNode: KnhMpIslandNode, inherited: String?) {
        fromNode.configuration.moduleDependencies
            ...
            .forEach { dependency ->
                ...
                if (result.putIfAbsent(island.module.path, KnhMpResolvedModuleDependency(configuration, island, other)) == null) {
...
internal fun Project.dependencyBuildTasks(target: KnhMpTarget, nodes: List<KnhMpIslandNode>): List<String> =
    nodes.flatMap { node ->
        resolvedModuleDependencies(target, node).map { "${it.island.module.path}:${it.island.buildTaskName(it.node)}" }
    }.distinct()
...
private fun stonecutterIslands(
    ...
    check(target.minecraftVersions.isNotEmpty()) { "Target ${target.name} declares no minecraft(...) versions" }
    val nodesByKey = target.minecraftVersions
        .map { version ->
            val configuration = extension.effectiveConfiguration(target, version)
            KnhMpIslandNode(version, ":$version", target.variantSourceSet(version), configuration)
        }
        .groupBy { node -> IslandKey(node.sourceSet, node.configuration.plugins.associate { it.id to it.version }) }

    val usedNames = mutableSetOf<String>()
    return nodesByKey.map { (key, nodes) ->
        val family = key.sourceSet.removePrefix(target.name).removeSuffix("Main").lowercase(Locale.ROOT)
...
internal fun Project.configureIslands(extension: KnhMpExtension, islands: List<KnhMpIsland>) {
    ...
    islands.forEach { island ->
        island.registerGradleBuild(island.ideClasspathTask, "knhmp") { listOf(island.activeNode().task(KnhMpIsland.EXPORT_TASK)) }
            .configure { it.dependsOn(dependencyBuildTasks(island.target, listOf(island.activeNode()))) }
    }

    val buildTasks = extension.targets.all().associateWith { target -> registerTargetTasks(target, islands.filter { it.target == target }) }

    val buildAll = tasks.register("buildAll", Sync::class.java) { task ->
        task.group = "build"
        task.description = "Builds every KnhMP target and variant in its island and collects the mod jars."
        task.into(layout.buildDirectory.dir("libs"))
        task.dependsOn(buildTasks.values)
        islands.forEach { island -> island.nodes.forEach { node -> task.from(island.jar(node)) } }
    }
...
private fun Project.registerTargetTasks(target: KnhMpTarget, islands: List<KnhMpIsland>): TaskProvider<*> {
    ...
    val islandBuilds = islands.map { island ->
        island.registerGradleBuild(if (islands.size > 1) "build${island.title}Island" else "build$title", "build") {
            island.nodes.map { it.task(target.buildTask) }
        }.also { it.configure { task -> task.dependsOn(dependencyBuildTasks(target, island.nodes)) } }
    }
    val buildTarget = if (islands.size == 1) islandBuilds.single() else tasks.register("build$title") { task ->
        task.group = "build"
    ...
    if (versioned) {
        islands.forEach { island ->
            island.nodes.forEach { node ->
                ...
                island.registerGradleBuild("build${title}_$suffix", "build") { listOf(node.task(target.buildTask)) }
                    .configure { it.dependsOn(upstream) }
                island.registerGradleBuild("run${title}_${suffix}Client", "knhmp") { listOf(node.task(target.runTask)) }
                    .configure { it.dependsOn(upstream) }
                if (island is KnhMpStonecutterIsland) {
                    island.registerGradleBuild("use${title}_$suffix", "knhmp") { listOf("Set active project to ${node.minecraftVersion}") }
                        .configure { task -> task.doLast { activeVariantFile(target).writeText(node.minecraftVersion) } }
                }
            }
        }
        val active = activeVariant(target, islands)
        active.island.registerGradleBuild("run${title}Client", "knhmp") { listOf(active.node.task(target.runTask)) }
            .configure { it.dependsOn(dependencyBuildTasks(target, listOf(active.node))) }
    } else {
        val island = islands.single()
        island.registerGradleBuild("run${title}Client", "knhmp") { listOf(island.nodes.single().task(target.runTask)) }
...
private fun Project.activeVariantFile(target: KnhMpTarget): File = projectDir.resolve(".knhmp/active-${target.name}")

/**
 * The variant unversioned run tasks follow. `use<Target>_<v>` records it; Stonecutter's own
 * per-island `stonecutter active` follows the same switch, so preprocessing and IDE classpaths agree.
 */
private fun Project.activeVariant(target: KnhMpTarget, islands: List<KnhMpIsland>): ActiveVariant {
    ...
    islands.forEach { island ->
        island.nodes.firstOrNull { it.minecraftVersion == recorded }?.let { return ActiveVariant(island, it) }
    }
