...
internal fun Project.registerJarVerification(extension: KnhMpExtension, islands: List<KnhMpIsland>) {
    tasks.register("verifyJvmTargets") { task ->
        task.group = "verification"
        task.description = "Checks inherited JVM targets and class-file versions in every target jar."
    ...
    tasks.register("verifySourceClosures") { task ->
        task.group = "verification"
        task.description = "Checks that every jar contains exactly its logical source closure and no sibling code."
        task.dependsOn("buildAll")
...
private fun Project.verifyJvmTargetModel(extension: KnhMpExtension) {
    ...
    extension.sourceSets.all().forEach { sourceSet ->
        ...
        val inheritedTarget = extension.sourceSets.closure(sourceSet.name())
            .asSequence()
...
private fun Project.verifyTargetJars(extension: KnhMpExtension, islands: List<KnhMpIsland>) {
    ...
    val sharedClasses = versionsByJar
        .map { (_, classVersions) -> classVersions.keys }
        .reduce(Set<String>::intersect)
    check(sharedClasses.isNotEmpty()) { "Target jars have no shared class produced from the canonical source closure" }
}
...
private fun Project.verifySourceClosures(extension: KnhMpExtension, islands: List<KnhMpIsland>) {
    val packagesBySourceSet = extension.sourceSets.names().associateWith { name ->
        listOf(projectDir.resolve("src/$name/kotlin"), javaSourceRoot(name)).flatMap { root ->
            root.walkTopDown()
    ...
    islands.forEach { island ->
        island.nodes.forEach { node ->
            ...
            val forbidden = (packagesBySourceSet.keys - closure).flatMap { packagesBySourceSet.getValue(it) }.toSet() - expected
            val jar = collectedJar(island, node)
            val packagesInJar = jar.classVersions().keys.map { it.substringBeforeLast('/', "") }.toSet()
            expected.forEach { pkg -> check(pkg in packagesInJar) { "${jar.name} lacks classes of package $pkg from ${node.sourceSet}'s closure" } }
            forbidden.forEach { pkg -> check(pkg !in packagesInJar) { "${jar.name} leaks classes of package $pkg outside its closure $closure" } }
            logger.lifecycle("{} <- {}", jar.name, closure.joinToString(" + "))
        }
    }
}

private fun File.classVersions(): Map<String, Int> = ZipFile(this).use { archive ->
    archive.entries().asSequence()
        .filter { !it.isDirectory && it.name.endsWith(".class") }
        .associate { entry ->
            val major = DataInputStream(archive.getInputStream(entry)).use { input ->
                check(input.readInt() == CLASS_FILE_MAGIC) { "${entry.name} is not a JVM class file" }
