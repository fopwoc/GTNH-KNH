...
/** Verifies the built artifacts, where registration/refmap failures are otherwise invisible to compilation. */
internal fun Project.registerMixinVerification(extension: KnhMpExtension, islands: List<KnhMpIsland>) {
    tasks.register("verifyMixinArtifacts") { task ->
        task.group = "verification"
        task.description = "Checks Mixin classes, loader registration, and refmaps in every enabled target jar."
        task.dependsOn("buildAll")
        task.doLast {
            islands.forEach { island ->
                island.nodes.filter { it.configuration.mixins != null }.forEach { node ->
...
private fun Project.verifyMixinArtifact(
    ...
    JarFile(jar).use { archive ->
        ...
        val containsMixinClass = archive.entries().asSequence()
            .any { !it.isDirectory && it.name.startsWith(classPrefix) && it.name.endsWith(".class") }
        check(containsMixinClass) {
            "${jar.name} contains no Mixin classes under $packageName"
        }

        val configNames = when (island.target.name) {
            "gtnh" -> {
                val expected = "mixins.${extension.modId}.json"
                val declared = archive.manifest.mainAttributes.getValue("MixinConfigs")
                    ?.split(',')
                    ?.map(String::trim)
                    .orEmpty()
                check(expected in declared) { "${jar.name} manifest does not register $expected: $declared" }
                listOf(expected)
            }
        ...
        if (island.target.name == "gtnh" ||
            island.target.name == "fabric" && KnhMpStonecutterIsland.isObfuscated(checkNotNull(node.minecraftVersion))
        ) {
            val expectedRefmap = if (island.target.name == "gtnh") {
            ...
            check(archive.getJarEntry(expectedRefmap) != null) { "${jar.name} lacks generated Mixin refmap $expectedRefmap" }
        }
    }
...
private fun fabricMixinConfigs(archive: JarFile): List<String> {
    ...
    return (metadata["mixins"] as? List<*>).orEmpty().map { declaration ->
        when (declaration) {
            ...
            is Map<*, *> -> checkNotNull(declaration["config"] as? String) { "Invalid Fabric Mixin declaration: $declaration" }
...
private fun neoForgeMixinConfigs(archive: JarFile): List<String> =
    MIXIN_TOML_BLOCK.findAll(archive.text("META-INF/neoforge.mods.toml")).map { it.groupValues[1] }.toList()
