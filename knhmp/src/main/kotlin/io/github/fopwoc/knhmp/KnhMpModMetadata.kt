...
internal object KnhMpModMetadata {
     ...
     * and a nested build.
     */
    fun generatedSourceRoot(project: Project): File =
        project.projectDir.resolve(".knhmp/generated/kotlin")

    fun generate(project: Project, extension: KnhMpExtension) {
        val file =
            generatedSourceRoot(project)
                .resolve(extension.modGroup.replace('.', '/'))
                .resolve("ModMetadata.kt")
        val content =
            """
...
internal fun Project.legacyJavaRoot(sourceSet: String): File =
    projectDir.resolve("src/$sourceSet/legacyJava")

/** Java source roots of a logical source set (mixins are Java); only the directory itself. */
internal fun Project.javaSourceRoot(sourceSet: String): File =
    projectDir.resolve("src/$sourceSet/java")
