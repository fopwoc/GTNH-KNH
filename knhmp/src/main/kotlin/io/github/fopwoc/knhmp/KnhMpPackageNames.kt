...
internal fun Project.verifyPackageNames(extension: KnhMpExtension) {
    val sources = extension.sourceSets.names().flatMap { name -> listOf("kotlin", "java").map { projectDir.resolve("src/$name/$it") } }
    val offenders =
        sources.filter { it.isDirectory }
            .flatMap { root -> root.walkTopDown().filter { it.isFile && (it.extension == "kt" || it.extension == "java") }.toList() }
            .mapNotNull { file ->
                val packageName = file.useLines { lines -> lines.firstNotNullOfOrNull { PACKAGE.find(it)?.groupValues?.get(1) } } ?: return@mapNotNull null
                val keywords = packageName.split('.').map { it.trim('`') }.filter { it in JAVA_KEYWORDS }
                if (keywords.isEmpty()) null else "${file.relativeTo(projectDir)}: package $packageName uses ${keywords.joinToString()}"
            }
            .distinct()
