...
class KnhMpPlugin : Plugin<Project> {
    ...
    override fun apply(project: Project) =
        with(project) {
            ...
            // Other modules consume this module's outputs (facade jars, collected island jars), so
            // a
            // `clean` in the same invocation must never run after them.
            tasks.configureEach { task ->
                if (!task.name.startsWith(BasePlugin.CLEAN_TASK_NAME))
    ...
    private fun validateTargetSourceSets(extension: KnhMpExtension) {
        extension.targets.all().forEach { target ->
            target.sourceSets().forEach { leaf ->
                check(leaf in extension.sourceSets.names()) {
                    "Target ${target.name} refers to unknown source set $leaf"
                }
            }
        }
    }
