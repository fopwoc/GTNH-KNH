...
/**
 * An island with one leaf source set and a Stonecutter tree holding one node per Minecraft version.
 * Stonecutter reads the canonical leaf source set through `src/main` symlinks, so switching the
 * active version rewrites the canonical files in place. Backends supply the node build script.
 */
internal abstract class KnhMpStonecutterIsland(
    ...
    fun activeVersion(): String {
        ...
        check(active in versions) {
            "Active Stonecutter version $active is not one of ${versions.joinToString()} in $directory"
        }
        return active
    }

    override fun activeNode(): KnhMpIslandNode = nodes.first {
        it.minecraftVersion == activeVersion()
    }
