...
object ClientWorldBackupManager {
    ...
    private val observedInventorySnapshots =
        linkedMapOf<InventoryLocation, ObservedInventorySnapshot>()
    ...
    fun canCaptureNow(): Boolean =
        resolveManualCaptureUnavailableReason(Minecraft.getMinecraft()) == null
    ...
    fun getStatusSnapshot(): BackupStatusSnapshot {
        val session = activeSession
        val currentDimensionId = Minecraft.getMinecraft().theWorld?.provider?.dimensionId
        val currentDimensionChunkCount =
            currentDimensionId?.let {
    ...
    fun getChunkHighlightSnapshot(dimensionId: Int): Map<Long, ChunkHighlightState> {
        ...
        return buildMap(savedChunkKeys.size) {
            savedChunkKeys.forEach { chunkKey ->
                put(
                    chunkKey,
                    if (sessionChunkKeys.contains(chunkKey)) ChunkHighlightState.SAVED_THIS_SESSION
                    else ChunkHighlightState.SAVED_EARLIER,
                )
            }
        }
    }
    ...
    fun onClientTick(event: TickEvent.ClientTickEvent) {
        ...
        if (BackupConfig.refreshIfChanged()) {
            ticksUntilNextCapture =
                ticksUntilNextCapture.coerceAtMost(
                    BackupConfig.autosaveIntervalSeconds * TICKS_PER_SECOND
                )
        }
    ...
    private fun captureObservedChunks(
        ...
        val stats =
            session.dimensionStats.getOrPut(dimensionId) {
                DimensionStats(
        ...
        if (savedThisPass > 0) {
            ...
            savedChunkCoordinates.forEach { (savedChunkX, savedChunkZ) ->
                patchChunkTileEntityData(
                    resolveDimensionDirectory(session.rootDirectory, dimensionId),
                    dimensionId,
                    savedChunkX,
                    savedChunkZ,
                )
            ...
            detailLine =
                "Tracked ${session.totalUniqueChunks()} unique chunk(s) in saves/${session.saveName}."
        } else {
            updateIdleStatus(world, session)
        }
    }
    ...
    private fun shouldSaveChunk(chunk: Chunk, chunkKeys: Set<Long>): Boolean {
        ...
        return chunk.isModified ||
            !chunkKeys.contains(chunkKey) ||
            !chunk.chunkTileEntityMap.isEmpty()
    }
    ...
    private fun writeStandaloneDimensionManifest(
        ...
        File(
                resolveStandaloneWorldDirectory(Minecraft.getMinecraft(), session, dimensionId),
                MANIFEST_FILE_NAME,
            )
    ...
    private fun resolveCaptureRadius(minecraft: Minecraft): Int {
        ...
        return when {
            ...
            minecraft.gameSettings.renderDistanceChunks > 0 ->
    ...
    private fun resolveSourceDescriptor(minecraft: Minecraft): BackupSourceDescriptor {
        ...
        if (serverData != null) {
            val displayName =
                serverData.serverName.takeIf { it.isNotBlank() } ?: serverData.serverIP
    ...
    private fun buildSaveName(source: BackupSourceDescriptor): String {
        ...
        val sourceName =
            sanitizeForFileName(source.address ?: source.displayName).ifBlank { UNKNOWN_SOURCE }
    ...
    private fun resolveStandaloneWorldDirectory(
        minecraft: Minecraft,
        session: BackupSession,
        dimensionId: Int,
    ): File {
        val standaloneSaveName =
            session.standaloneSaveNamesByDimension.getOrPut(dimensionId) {
    ...
    private fun captureObservedInventories(minecraft: Minecraft, world: WorldClient) {
        ...
        currentScreen.inventorySlots.inventorySlots.filterIsInstance<Slot>().forEach { slot ->
            ...
            groupedSnapshots.getOrPut(location) { linkedMapOf() }[slot.slotIndex] =
                ItemStack.copyItemStack(stack).writeToNBT(NBTTagCompound())
        }

        groupedSnapshots.forEach { (location, items) ->
            observedInventorySnapshots[location] =
                ObservedInventorySnapshot(items = items.toMap(), explicitlyObserved = true)
        }
    }
    ...
    private fun patchChunkTileEntityData(
        chunkDirectory: File,
        dimensionId: Int,
        chunkX: Int,
        chunkZ: Int,
    ...
    private fun mergeInventorySnapshotIntoTileEntity(
        tileEntityTag: NBTTagCompound,
        snapshot: ObservedInventorySnapshot,
    ): Boolean {
    ...
    private fun mergeTileEntitySnapshotIntoTag(
        tileEntityTag: NBTTagCompound,
        liveSnapshot: NBTTagCompound,
    ): Boolean {
        var mutated = false
        @Suppress("UNCHECKED_CAST") val keys = liveSnapshot.func_150296_c() as Set<String>
    ...
    private fun readChunkNbt(chunkDirectory: File, chunkX: Int, chunkZ: Int): NBTTagCompound? {
        val input =
            RegionFileCache.getChunkInputStream(chunkDirectory, chunkX, chunkZ) ?: return null
        return input.use(CompressedStreamTools::read)
    }

    private fun writeChunkNbt(
        chunkDirectory: File,
        chunkX: Int,
        chunkZ: Int,
        chunkRoot: NBTTagCompound,
    ) {
    ...
    private fun initializeKnownChunks(session: BackupSession) {
        ...
        knownChunks.forEach { (dimensionId, chunkKeys) ->
            ...
            session.dimensionStats[dimensionId] =
                DimensionStats(
                    dimensionId = dimensionId,
                    folder = resolveDimensionFolderName(dimensionId),
                    firstSeenEpochMillis = System.currentTimeMillis(),
    ...
    private fun loadKnownChunksFromDisk(rootDirectory: File): Map<Int, Set<Long>> {
        ...
        rootDirectory
            ...
            ?.forEach { dimensionDirectory ->
                val dimensionId =
                    dimensionDirectory.name.removePrefix("DIM").toIntOrNull() ?: return@forEach
    ...
    private fun scanDimensionChunks(dimensionDirectory: File, dimensionId: Int): Set<Long>? {
        ...
        val regionFiles =
            regionDirectory.listFiles { file ->
    ...
    private fun resolveInitialSpawn(
        ...
        val savedOverworldChunk =
            session.savedChunksByDimension[SAFE_OVERWORLD_DIMENSION_ID]?.firstOrNull()
