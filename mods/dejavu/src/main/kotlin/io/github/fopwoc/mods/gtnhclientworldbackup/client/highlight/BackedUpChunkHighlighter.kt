...
object BackedUpChunkHighlighter {
    ...
    fun onRenderWorldLast(event: RenderWorldLastEvent) {
        ...
        val savedChunks =
            ClientWorldBackupManager.getChunkHighlightSnapshot(world.provider.dimensionId)
        ...
        val nearbyChunks = savedChunks.filterKeys { chunkKey ->
            if (BackupConfig.highlightOnlyTargetedChunk) {
                ...
                isChunkWithinRadius(
                    ...
                    player.chunkCoordX,
                    player.chunkCoordZ,
                    BackupConfig.highlightRenderRadiusChunks,
                )
            }
        }
    ...
    private fun renderChunkHighlight(
        ...
        val fillAlpha =
            (if (targeted) BackupConfig.highlightFillAlpha * 1.75f
                else BackupConfig.highlightFillAlpha)
                .coerceAtMost(0.5f)
        val outlineAlpha =
            (if (targeted) BackupConfig.highlightOutlineAlpha
                else BackupConfig.highlightOutlineAlpha * 0.85f)
                .coerceIn(0.05f, 1.0f)
    ...
    private fun isChunkWithinRadius(
        chunkKey: Long,
        centerChunkX: Int,
        centerChunkZ: Int,
        radius: Int,
