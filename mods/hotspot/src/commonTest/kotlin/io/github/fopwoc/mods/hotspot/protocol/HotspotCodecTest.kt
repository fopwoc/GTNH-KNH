    ...
    fun snapshotPartRoundTripsWithSharedNames() {
        val chunk =
            ChunkProfile(
                ...
                tileEntities =
                    listOf(
                        TileEntityProfile(-1241, 68, 391, 2.25, "Large Steam Turbine", "GT_MetaTileEntity"),
        ...
        assertEquals(ProfileSnapshotPart(1, 2, 3, 0, true, null), ProfileSnapshotPartCodec.roundTrip(ProfileSnapshotPart(1, 2, 3, 0, true, null)))
    }

    @Test
    fun truncatedAndOutOfRangePayloadsAreRejected() {
        val truncated = MessageWriter().long(1).long(2).int(20).int(0).byte(2).toByteArray()
        assertFailsWith<MalformedMessageException> { ProfileSnapshotPartCodec.decode(MessageReader(truncated)) }

        val outOfRange = MessageWriter().long(1).int(MAX_DURATION_TICKS + 1).toByteArray()
        assertFailsWith<MalformedMessageException> { ProfileRequestCodec.decode(MessageReader(outOfRange)) }
    }
        ...
        val chunks =
            (0 until 300).map { index ->
                ChunkProfile(
                    ...
                    tileEntityCount = 128,
                    entityCount = 0,
                    tileEntities = (0 until 128).map { i -> TileEntityProfile(index * 16 + i % 16, 64, i / 16, 0.01, "Machine $i", "Class${i % 7}") },
                )
            }
        val parts = ProfileSnapshotParts.split(ProfileSnapshot(1, 0, 100, listOf(DimensionProfile(0, "Overworld", 10.0, chunks))))
