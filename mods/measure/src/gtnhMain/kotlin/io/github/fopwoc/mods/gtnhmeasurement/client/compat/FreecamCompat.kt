...
object FreecamCompat {
    ...
    /**
     * Freecam moves its camera from the raw keyboard in its own client tick, so a key-state
     * override cannot stop it descending on Shift. Instead: [rememberHeight] at the start of the
     * tick, and [restoreHeight] before the frame is rendered puts the camera back and kills its
     * vertical momentum whenever [hold] is true — the camera never visibly moves.
     */
    fun rememberHeight() {
    ...
    private class HeightLock(
        ...
        fun setCameraY(y: Double) {
            ...
            // setPosition also moves the bounding box, which is what moveEntity derives posY from
            // on
