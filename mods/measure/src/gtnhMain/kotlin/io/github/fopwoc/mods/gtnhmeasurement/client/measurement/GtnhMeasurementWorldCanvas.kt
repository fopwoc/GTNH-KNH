...
internal class GtnhMeasurementWorldCanvas(private val scope: WorldOverlayScope) :
    MeasurementWorldCanvas {
    override val eyeX: Double
        get() = scope.camera.eyeX

    override val eyeY: Double
        get() = scope.camera.eyeY

    override val eyeZ: Double
        get() = scope.camera.eyeZ
    ...
    override fun blockOutline(x: Int, y: Int, z: Int, color: Color, width: Float) =
        scope.blockOutline(x, y, z, color, width)
