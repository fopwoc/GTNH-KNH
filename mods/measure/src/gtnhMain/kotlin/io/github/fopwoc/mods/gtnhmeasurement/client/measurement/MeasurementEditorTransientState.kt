...
internal class MeasurementEditorTransientState {
    ...
    fun hasActiveDraftCreation(isPastePlacementActive: Boolean): Boolean =
        draftFirstSelection != null && !isPastePlacementActive
    ...
    fun syncForDimension(currentDimensionId: Int, visibleIds: Set<Long>) {
        ...
        selectionCycleState = selectionCycleState?.takeIf { cycle ->
            cycle.anchor.isInDimension(currentDimensionId) &&
                cycle.candidateIds.any { it in visibleIds }
        }
    }

    fun nextCycledMeasurement(
        ...
        val index =
            if (
                existingCycle != null &&
                    existingCycle.anchor == anchor &&
                    existingCycle.candidateIds == candidateIds
            ) {
