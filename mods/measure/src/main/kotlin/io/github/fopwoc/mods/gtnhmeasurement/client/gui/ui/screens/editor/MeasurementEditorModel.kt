...
data class MeasurementEditorModel(
    ...
    val availableModes: List<MeasurementMode> = listOf(
        MeasurementMode.LINE,
        MeasurementMode.AREA,
        MeasurementMode.SPHERE,
        MeasurementMode.DISABLED
    ),
    val summary: String = "Choose a measurement mode and inspect live editor status while the world interaction layer stays active.",
    ...
    val historyStatusText: String = "Undo ${if (canUndo) "ready" else "empty"} · Redo ${if (canRedo) "ready" else "empty"}"

    val hoverStatusText: String = when (hoverKind) {
