...
object MeasurementEditorRuntimeSnapshot {
    fun read(minecraft: Minecraft = Minecraft.getMinecraft()): MeasurementEditorModel {
        ...
        val visibleMeasurements = currentDimensionId?.let(MeasurementSelectionState::measurementsForDimension).orEmpty()
        val selectedMeasurements = currentDimensionId?.let(MeasurementSelectionState::selectedMeasurementsForDimension).orEmpty()
        val previewMeasurements = currentDimensionId?.let(MeasurementSelectionState::previewMeasurementsForDimension).orEmpty()
        ...
        return MeasurementEditorModel(
            ...
            hoverTargetText = hoveredTarget?.block?.let { block ->
    ...
    private fun draftStatusText(): String {
        ...
        return when {
            ...
            first != null && second != null -> "Draft from ${formatBlock(first)} to ${formatBlock(second)}"
    ...
    private fun formatBlock(block: io.github.fopwoc.mods.gtnhmeasurement.client.measurement.BlockSelection): String =
        "(${block.x}, ${block.y}, ${block.z})"
}
