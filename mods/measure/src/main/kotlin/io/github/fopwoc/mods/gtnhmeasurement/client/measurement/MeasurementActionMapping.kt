...
object MeasurementActionMapping {
    fun resolveKeyboardActions(snapshot: MeasurementInputSnapshot): List<MeasurementKeyboardAction> = buildList {
        if (snapshot.escapeTriggered) {
            add(MeasurementKeyboardAction.CANCEL_ACTIVE_INTERACTION)
            return@buildList
        }
        when {
            snapshot.redoPrimaryTriggered || snapshot.redoSecondaryTriggered -> add(MeasurementKeyboardAction.REDO)
    ...
    ): MeasurementWorldClickAction = when {
        isPastePlacementActive && !snapshot.transformModifierDown -> {
            MeasurementWorldClickAction.PLACE_CLIPBOARD
        }
        hasActiveDraftCreation && !snapshot.transformModifierDown -> MeasurementWorldClickAction.REGISTER_ANCHOR
        snapshot.selectionModifierDown && snapshot.targetModifierDown -> MeasurementWorldClickAction.SELECT_MULTI
