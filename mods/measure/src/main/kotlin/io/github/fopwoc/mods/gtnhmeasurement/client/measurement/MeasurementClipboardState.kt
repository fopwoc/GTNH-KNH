...
internal class MeasurementClipboardState {
  ...
  fun clearTransformClipboardIfNeeded() {
    if (
        clipboard?.operation == ClipboardOperation.MOVE ||
            clipboard?.operation == ClipboardOperation.RESIZE
    ) {
      clipboard = null
    }
  }
  ...
  fun moveFrom(
      originAnchor: BlockSelection,
      measurements: List<PersistedMeasurement>,
      sourceMeasurementIds: Set<Long>,
  ) {
    clipboard =
        MeasurementClipboard(
  ...
  fun resizeFrom(
      ...
      resizeAnchorRole: MeasurementAnchorRole,
  ) {
    clipboard =
        MeasurementClipboard(
  ...
  fun transformedClipboard(
      anchor: BlockSelection,
      activeClipboard: MeasurementClipboard,
  ...
  fun restoreFromHistory(
      snapshotClipboard: MeasurementClipboard?,
      snapshotLastAnchorInteraction: BlockSelection?,
  ) {
