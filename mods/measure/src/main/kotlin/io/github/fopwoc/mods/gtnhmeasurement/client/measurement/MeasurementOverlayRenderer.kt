...
object MeasurementOverlayRenderer {
  ...
  fun onRenderWorld(event: RenderWorldLastEvent) {
    ...
    persistedMeasurements
        .filter { MeasurementSelectionState.isSelected(it.id) }
        .forEach { measurement ->
          drawMeasurement(
              mode = measurement.mode,
              first = measurement.first,
  ...
  private fun drawMeasurementShape(
    ...
    when (mode) {
      MeasurementMode.LINE ->
          drawLine(first, second, style.lineColor, style.shapeWidth, cameraX, cameraY, cameraZ)
      MeasurementMode.AREA ->
          drawAreaOutline(
              first,
              second,
  ...
  private fun drawLine(
    ...
    GL11.glVertex3d(
        second.centerX() - cameraX,
        second.centerY() - cameraY,
        second.centerZ() - cameraZ,
    )
    GL11.glEnd()
  }
  ...
  private fun rgb(color: Color): Triple<Float, Float, Float> =
      Triple(
          color.red / 255.0f,
          color.green / 255.0f,
          color.blue / 255.0f,
