...
object MeasurementPersistence {
    ...
    private fun resolveWorldName(world: World): String? = runCatching {
        world.worldInfo.worldName
    }.getOrNull()?.takeIf(String::isNotBlank)

    private fun storageFile(contextId: String): File = JsonFileStorage.modConfigFile(
    ...
    private fun resolveServerDescriptor(minecraft: Minecraft): String? = runCatching {
        val accessor = minecraft.javaClass.methods.firstOrNull { method ->
            method.parameterCount == 0 && (method.name == "func_147104_D" || method.name == "getCurrentServerData")
    ...
    private fun readStringProperty(instance: Any, propertyName: String): String? = runCatching {
        val field = instance.javaClass.fields.firstOrNull { it.name == propertyName } ?: return@runCatching null
