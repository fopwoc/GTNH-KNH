...
object MeasurementSelectionState {
    ...
    fun exportPersistedMeasurements(): List<PersistedMeasurement> = store.exportPersistedMeasurements()
    ...
    fun registerMeasurementAnchor(
        ...
        val resolvedSecond = resolveDraftSecondAnchor(
            first = first,
            clicked = clicked,
            mode = mode,
        ...
        val added = store.addMeasurement(
    ...
    private fun createSnapshot(): MeasurementEditorSnapshot = MeasurementEditorSnapshot(
    ...
    private fun resolvePlacementAnchor(anchor: BlockSelection, constrainToRightAngles: Boolean): BlockSelection {
        ...
        return when (activeClipboard.operation) {
            ...
            ClipboardOperation.MOVE -> MeasurementGeometry.snapToRightAngle(activeClipboard.originAnchor, anchor)
        }
    }

    private fun resolveResizePlacementAnchor(anchor: BlockSelection, clipboard: MeasurementClipboard): BlockSelection {
        val measurement = clipboard.measurements.singleOrNull() ?: return anchor
        if (measurement.mode != MeasurementMode.LINE) {
            return anchor
        }

        val fixedAnchor = when (clipboard.resizeAnchorRole) {
