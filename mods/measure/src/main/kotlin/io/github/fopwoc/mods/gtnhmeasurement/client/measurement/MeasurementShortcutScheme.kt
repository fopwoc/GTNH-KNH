...
object MeasurementShortcutScheme {
    val platformProfile: MeasurementPlatformProfile = when {
        System.getProperty("os.name")
            ?.lowercase()
            ?.contains("mac") == true -> MeasurementPlatformProfile.MAC
        else -> MeasurementPlatformProfile.STANDARD
    }

    fun selectionModifierLabel(): String = if (platformProfile == MeasurementPlatformProfile.MAC) "⇧" else "Shift"

    fun constraintModifierLabel(): String = selectionModifierLabel()

    fun targetModifierLabel(): String = if (platformProfile == MeasurementPlatformProfile.MAC) "⌃" else "Ctrl"

    fun transformModifierLabel(): String = if (platformProfile == MeasurementPlatformProfile.MAC) {
        "⌥"
    } else {
        "Alt"
    }

    fun editorModifierLabel(): String = if (platformProfile == MeasurementPlatformProfile.MAC) {
        "⌘"
    } else {
        "Ctrl"
    }
    ...
    fun deleteLabel(): String = if (platformProfile == MeasurementPlatformProfile.MAC) "⌫" else "Del/Backspace"
    ...
    fun redoLabel(): String = if (platformProfile == MeasurementPlatformProfile.MAC) {
    ...
    fun editorModifierDown(): Boolean = if (platformProfile == MeasurementPlatformProfile.MAC) {
    ...
    fun currentKeyboardSnapshot(keyPressed: (Int) -> Boolean): MeasurementInputSnapshot {
        ...
        return MeasurementInputSnapshot(
            ...
            redoPrimaryTriggered = when (platformProfile) {
                MeasurementPlatformProfile.MAC -> editorModifierDown && selectionModifierDown && keyPressed(Keyboard.KEY_Z)
                MeasurementPlatformProfile.STANDARD -> editorModifierDown && keyPressed(Keyboard.KEY_Y)
            },
            redoSecondaryTriggered = platformProfile == MeasurementPlatformProfile.STANDARD &&
    ...
    fun currentWorldClickSnapshot(): MeasurementInputSnapshot = MeasurementInputSnapshot(
        selectionModifierDown = selectionModifierDown(),
        targetModifierDown = targetModifierDown(),
        transformModifierDown = transformModifierDown(),
        editorModifierDown = editorModifierDown()
    ...
    fun redoPrimaryPressed(keyPressed: (Int) -> Boolean): Boolean = when {
        platformProfile == MeasurementPlatformProfile.MAC -> editorModifierDown() && selectionModifierDown() && keyPressed(Keyboard.KEY_Z)
    ...
    fun footerText(): String = if (platformProfile == MeasurementPlatformProfile.MAC) {
    ...
    private fun chord(vararg keys: String): String = when (platformProfile) {
