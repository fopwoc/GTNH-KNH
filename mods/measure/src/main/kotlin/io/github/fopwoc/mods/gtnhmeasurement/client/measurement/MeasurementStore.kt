...
internal class MeasurementStore {
  ...
  fun measurementsForAnchor(anchor: BlockSelection): List<MeasurementRecord> =
      measurements.filter { it.containsAnchor(anchor) }.sortedBy(MeasurementRecord::id)

  fun measurementsContainingBlock(block: BlockSelection): List<MeasurementRecord> =
      measurements.filter { it.containsBlock(block) }.sortedBy(MeasurementRecord::id)
  ...
  fun exportPersistedMeasurements(): List<PersistedMeasurement> =
      measurements.map(MeasurementRecord::toPersisted)
  ...
  fun addMeasurement(measurement: PersistedMeasurement, markDirty: Boolean = true): Boolean {
    if (
        !measurement.mode.isEnabled ||
            measurement.first.dimensionId != measurement.second.dimensionId
    ) {
      return false
    }
  ...
  fun snapshot(): MeasurementStoreSnapshot =
      MeasurementStoreSnapshot(
