...
import io.github.fopwoc.mods.framework.minecraft.forward
import io.github.fopwoc.mods.framework.minecraft.id
import io.github.fopwoc.mods.framework.minecraft.mainCamera
...
object MeasurementHoverResolver {
    fun resolve(
        ...
        val dimensionId = level.dimension().id.toString()
        val camera = minecraft.mainCamera
        val origin = camera.eye
        val look = camera.forward
        val direction = Vec3(look.x().toDouble(), look.y().toDouble(), look.z().toDouble())
        val reach =
            if (viewer !== player) ModernFreecamReach.reach.toDouble()
            else player.blockInteractionRange()
        ...
        val pick =
            MeasurementRayPicker.pick(
                ...
                isLoaded = { x, y, z ->
                    y in level.bottomY until level.topY && level.hasChunk(x shr 4, z shr 4)
                },
        ...
        if (usePlacementOffset) {
            ...
            if (hit.type == HitResult.Type.BLOCK) {
                ...
                if (
                    !level.isOutsideBuildHeight(adjacent) &&
                        level.hasChunk(adjacent.x shr 4, adjacent.z shr 4)
                ) {
