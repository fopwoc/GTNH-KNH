...
object MeasurementShortcutHudOverlay : HudLayer("measure") {
    ...
    override val visible: Boolean
        get() {
            ...
            return MeasurementSession.isActive && MeasurementConfig.showShortcutHud &&
                client.gui.screen() == null && !client.gui.hud.isHidden && ClientBackend.current.isInWorld
        }
    ...
    override fun beforeFrame() {
        ...
        model = MeasurementShortcutHudResolver.resolve(
            MeasurementShortcutHudContext(
                modeActive = MeasurementSession.isActive,
                selectedMode = MeasurementSession.mode,
                selectedMeasurementCount = MeasurementSelectionState.selectedMeasurementsForDimension(dimensionId).size,
                hoveredMeasurementCount = hovered?.let(MeasurementSelectionState::measurementsContainingBlock)?.size ?: 0,
                ...
                freecamReach = ModernFreecamReach.reach.takeIf { ModernFreecamReach.isDetached },
            )
        )
        val current = model ?: return
        val font = client.font
        val rowHeight = font.lineHeight + CHIP_PADDING_Y * 2
        val rowWidths = current.hints.map { hint ->
            (if (hint.keys.isEmpty()) 0 else font.width(hint.keys) + CHIP_PADDING_X * 2 + 2 + ROW_GAP) + font.width(hint.action)
        }
        val boxWidth = maxOf(font.width(current.title), rowWidths.maxOrNull() ?: 0) + BOX_PADDING * 2 + 2
        val boxHeight = font.lineHeight + TITLE_GAP + current.hints.size * rowHeight +
            (current.hints.size - 1).coerceAtLeast(0) * ROW_SPACING + BOX_PADDING * 2 + 2
        bounds = HudRect(width / 2 - boxWidth / 2, height - 22 - MeasurementConfig.hudMargin - boxHeight, boxWidth, boxHeight)
    }
    ...
    override fun Content() {
        ...
        Box(modifier = Modifier.fillMaxSize()) {
            HudAnchor(bounds = bounds, contentAlignment = Alignment.TopStart) {
                Box(modifier = Modifier.fillMaxSize().background(Color(0x7C000000)).border(Color(0xAA4A4A56))) {
                    Column(
