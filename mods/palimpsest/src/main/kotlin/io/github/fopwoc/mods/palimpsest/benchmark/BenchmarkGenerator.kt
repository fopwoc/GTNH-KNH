...
/** Reproducible sparse rebuilds over a fixed synthetic 32×32 chunk world. */
internal object BenchmarkGenerator {
  ...
  data class Result(val layersWritten: Int, val elapsedNanos: Long, val bytesAdded: Long)

  fun append(store: TileHistoryStore): Result {
    ...
    for (epoch in firstEpoch until firstEpoch + EPOCHS_PER_BATCH) {
      ...
      for (key in changedTiles.sortedWith(compareBy(TileKey::z, TileKey::x))) {
        ...
        while (positions.size < CHANGED_CELLS_PER_TILE) positions += random.nextInt(next.size)
        for (position in positions) {
          next[position] = ((next[position].toInt() and 255) + 1 + random.nextInt(48)).toByte()
        }
    ...
    store.append(layers)
    return Result(layers.size, System.nanoTime() - started, store.byteCount - previousBytes)
  }
}
