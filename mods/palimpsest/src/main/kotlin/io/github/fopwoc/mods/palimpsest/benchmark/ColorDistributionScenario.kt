...
internal object ColorDistributionScenario {
  ...
  fun run(directory: Path, shouldStop: () -> Boolean = { false }): List<Result>? {
        ...
        listOf(
            ...
            Pattern("near-uniform") { tile ->
              ByteArray(TileLayer.PIXELS) { position ->
                ((tile * 37 + if (position == (tile * 17 and 255)) 1 else 0) and 255).toByte()
              } to null
            },
            Pattern("terrain-bands") { tile -> bands(tile) to null },
            Pattern("varied") { tile ->
              ByteArray(TileLayer.PIXELS) { position ->
                ((tile * 73 + position * 29 + (position / TileLayer.SIDE) * 17) and 255).toByte()
              } to null
            },
            Pattern("solid-footprint") { tile ->
              val first = bands(tile)
              first to first.copyOf().apply {
                for (z in 4 until 12) for (x in 4 until 12) this[z * TileLayer.SIDE + x] = 200.toByte()
              }
            },
            Pattern("scattered-solid") { tile ->
              val first = bands(tile)
              first to first.copyOf().apply {
                repeat(64) { index -> this[(index * 37 + tile * 13) and 255] = 200.toByte() }
              }
            },
    ...
    for (pattern in patterns) {
      ...
      val plainBytes =
          12L + TILES * 12L + layers.sumOf { layer ->
      ...
      val result = TileHistoryStore(directory.resolve(pattern.name)).use { store ->
        ...
        Result(pattern.name, TILES, layers.size, append.bytesAdded, plainBytes, sampledBytes, sampleNanos)
      }
