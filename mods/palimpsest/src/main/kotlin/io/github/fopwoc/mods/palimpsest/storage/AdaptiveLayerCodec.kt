...
internal object AdaptiveLayerCodec {
                ...
                MASKED -> {
                    ...
                    val count =
                        (0 until TileLayer.MASK_WORDS).sumOf {
                            java.lang.Long.bitCount(buffer.long)
                        }
                    if (count !in 1 until TileLayer.PIXELS)
                        throw IOException("Invalid masked coverage")
    ...
    fun encode(layer: TileLayer, epochDelta: Long): ByteArray {
        ...
        val dominant =
            if (layer.colors.size == TileLayer.PIXELS) dominantColor(layer.colors) else -1
        val kind =
            when {
        ...
        val buffer =
            ByteBuffer.allocate(1 + varLongSize(epochDelta) + payload)
        ...
        when (kind) {
            SPARSE -> {
                ...
                repeat(count) { index ->
                    ...
                    coverage[position ushr 6] =
                        coverage[position ushr 6] or (1L shl (position and 63))
            ...
            FULL -> {
                if (buffer.remaining() != TileLayer.PIXELS)
                    throw IOException("Invalid full layer length")
                coverage.fill(-1L)
