...
/**
 * Frequency counts over a small alphabet that adapt as symbols are coded, identical on both sides,
 * so a record needs no table. Counts start at one; when the total reaches [LIMIT] every count
 * halves, which keeps the coder's precision and lets the model follow local statistics.
 */
