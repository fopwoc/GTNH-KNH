 ...
 */
class RootIndex
private constructor(private val epochs: LongArray, private val roots: Array<RootRecord>) {
    constructor(
        roots: List<RootRecord>
    ) : this(
        roots.sortedBy { it.epoch }.map { it.epoch }.toLongArray(),
        roots.sortedBy { it.epoch }.toTypedArray(),
    )
    ...
    /**
     * The root in force at [epoch]: the newest one committed at or before it; empty before the
     * first.
     */
