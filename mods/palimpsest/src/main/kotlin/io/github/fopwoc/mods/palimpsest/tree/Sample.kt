...
/**
 * One pixel's facts — block id, height, liquid depth, biome — packed in a Long. This is what a
 * node keeps per child and what a page samples at far zoom; [NONE] marks a child that does not
 * exist.
 */
@JvmInline
value class Sample(val packed: Long) {
    constructor(block: Int, height: Int, depth: Int, biome: Int) :
        this(
    ...
    override fun toString(): String =
        if (isNone) "Sample(none)" else "Sample(block=$block height=$height depth=$depth biome=$biome)"
