...
/**
 * Spells refs the way a segment file must. The first varint says where: 0 is null, 1 is this
 * very segment (the common case: the record written a moment ago), and `slot + 2` names a machine
 * in this segment's slot table followed by that machine's segment ordinal. Reading resolves back
 * to runtime segment indices through the [SegmentSet]; a ref into a segment this machine has
 * never seen is corruption (a sync that dropped files).
 */
class SlotRefCoder(private val segments: SegmentSet, private val index: Int, private val reader: SegmentReader) : RefCoder {
    ...
    override fun read(source: ByteSource): Ref {
        ...
        if (slot >= slots.size) throw CorruptTreeException("Slot $slot beyond the segment's ${slots.size}")
        ...
        if (segment < 0) throw CorruptTreeException("Ref into unknown segment ${MachineId.hex(machine)}#$ordinal")
