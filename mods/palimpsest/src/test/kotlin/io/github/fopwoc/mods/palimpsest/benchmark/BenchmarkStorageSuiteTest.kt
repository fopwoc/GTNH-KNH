...
class BenchmarkStorageSuiteTest {
    ...
    fun horizontalScalingReadsBoundedSamplesAcrossLods() {
        ...
        try {
            ...
            for (line in colors) {
                val sizes =
                    colorSizePattern.findAll(line).map { it.groupValues[1].toLong() }.toList()
                assertTrue(sizes[0] <= sizes[1], line)
            }
            val byPattern = colors.associateBy {
                Regex("pattern=([^ ]+)").find(it)!!.groupValues[1]
            }
            ...
            val bytes =
                Regex("logical_record_bytes_read=(\\d+)").find(report)!!.groupValues[1].toLong()
