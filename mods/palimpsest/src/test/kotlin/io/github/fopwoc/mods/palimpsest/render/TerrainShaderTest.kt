...
class TerrainShaderTest {
    ...
    private val shader = TerrainShader({ colors.getValue(it) }, { it == 4 }, { biome -> if (biome == 6) 0x80FF80 else 0xFFFFFF })

    private fun shaded(color: Int, shade: Int) = TerrainShader.shade(color or (0xFF shl 24), TerrainShader.SHADES[shade])
    ...
    private fun render(grid: SampleGrid): IntArray {
        ...
        return IntArray(grid.side * grid.side) { pixel ->
            val at = pixel * 4
            ((rgba[at + 3].toInt() and 255) shl 24) or ((rgba[at].toInt() and 255) shl 16) or ((rgba[at + 1].toInt() and 255) shl 8) or (rgba[at + 2].toInt() and 255)
        }
    }
    ...
    fun flatGroundIsTheMiddleShadeAndAbsentCellsAreTransparent() {
        ...
        assertEquals(shaded(0x808080, 1), pixels[0])
    ...
    @Test
    fun waterShadesByDepthAndTintableBlocksTakeTheBiomeColor() {
        val grid = SampleGrid(4)
        for (z in 0 until 4) for (x in 0 until 4) grid.set(x, z, 3, 62, 1 + x * 5, 1)
        val pixels = render(grid)
        assertEquals(shaded(0x3F5FDF, 2), pixels[0])
        assertEquals(shaded(0x3F5FDF, 0), pixels[3])
        ...
        assertEquals(shaded(TerrainShader.applyTint(0xFF939393.toInt(), 0x80FF80), 1), tintedPixels[0])
