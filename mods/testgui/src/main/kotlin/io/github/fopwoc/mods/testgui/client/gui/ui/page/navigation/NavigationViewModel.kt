...
class NavigationViewModel : ViewModel() {
  ...
  fun recordSelfPush() {
    stateFlow.update { state ->
      state.copy(
          ...
          outerEvents =
              prepend("Pushed another Navigation entry on the outer stack", state.outerEvents),
      )
    }
  }
  ...
  private fun prepend(message: String, log: List<String>): List<String> =
      listOf(message) + log.take(5)
}
