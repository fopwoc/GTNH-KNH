...
fun OverviewView(
  ...
  Column(
    ...
    SectionBlock(title = "Screen list", modifier = Modifier.fillMaxWidth()) {
      BodyText(
          "This demo app opens each framework area as its own full-screen route. The list stays at the root so Back always returns here."
      )
    ...
    state.features.forEach { feature ->
      SectionBlock(
          title = feature.title,
          modifier = Modifier.fillMaxWidth().tooltip(feature.summary),
          elevated = true,
      ) {
        Row(
          ...
          Button(
              text = "Open",
              modifier =
                  Modifier.tooltip("Open ${feature.title} full-screen and return here with Back."),
              onClick = {
        ...
        if (feature == state.features.last()) {
          BodyText(
              "The newest stress routes are meant for intentionally uncomfortable UI conditions: dense hosted widgets, focus churn, long scroll chains, and offset badge pressure."
          )
        } else {
          BodyText(
              "Open this route to validate one framework surface in isolation, then back out to the menu list."
          )
        }
      }
    }
