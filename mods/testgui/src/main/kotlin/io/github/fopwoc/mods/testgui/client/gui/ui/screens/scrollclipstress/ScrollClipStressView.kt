    ...
    Column(
        ...
        SectionBlock(title = "Stress controls", modifier = Modifier.fillMaxWidth()) {
            Row(
                ...
                Button(text = "+ badge offset", modifier = Modifier.weight(1f), onClick = onIncreaseBadgeOffset)
                Button(text = "- badge offset", modifier = Modifier.weight(1f), onClick = onDecreaseBadgeOffset)
            }
            Row(
                modifier = Modifier.fillMaxWidth(),
        ...
        SectionBlock(title = "Scrollable clip pressure", modifier = Modifier.weight(1f).fillMaxWidth(), elevated = true) {
            Column(
                modifier = Modifier
                    .fillMaxSize()
                ...
                repeat(state.laneCount) { index ->
                    val badgeLeft = if (state.alternatingBadges && index % 2 == 1) state.badgeOffset / 2 else state.badgeOffset
                    ...
                    Box(
                        ...
                        Box(
                            modifier = Modifier
                                .size(width = 68.uu, height = (laneHeight + 8).uu)
        ...
        BodyText("Use this route to inspect whether long scrolled regions stay stable while partially offset badges pressure clipping and hit testing.")
