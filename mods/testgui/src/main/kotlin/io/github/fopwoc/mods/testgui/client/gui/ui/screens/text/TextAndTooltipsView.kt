...
fun TextAndTooltipsView(
    ...
    Tabs(
        ...
        when (tab) {
            ...
            TextDemoTab.Styled -> StyledTextTab(
...
private fun WrappedTextTab() {
    Panel(modifier = Modifier.fillMaxWidth()) {
        Column(
            ...
            Text(
                text = "The Text node supports alignment, color, wrap and shadow. This paragraph is intentionally long enough to wrap across multiple lines so the layout engine can be checked in a realistic panel width.",
                modifier = Modifier.fillMaxWidth(),
...
private fun TooltipsTab() {
    Panel(
        modifier = Modifier
            .fillMaxWidth()
            .tooltip(
                StyledText.of("Hover anywhere inside this panel to validate container-level tooltips.")
            )
    ) {
        Column(
            modifier = Modifier.fillMaxWidth(),
            ...
            Text(
                text = "Hover the panel, then hover the button below to make sure the topmost target wins.",
                modifier = Modifier.fillMaxWidth(),
                style = TextStyle(wrap = true)
            )
            Button(
...
private fun styledAccentLabel(accentPasses: Int): StyledText {
    val cycleLabel = when (accentPasses % 3) {
