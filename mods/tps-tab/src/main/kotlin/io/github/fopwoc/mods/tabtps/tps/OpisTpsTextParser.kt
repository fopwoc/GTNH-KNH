...
object OpisTpsTextParser {
  ...
  fun parse(lines: List<String>, descriptor: DimensionDescriptor?): ParsedOpisReport {
    ...
    val overallLine =
        parsedLines.firstOrNull { line -> containsOverallKeyword(line.normalized) }
  ...
  private fun extractValue(rawLine: String, patterns: List<Regex>): Double? {
    return patterns
        .asSequence()
        .mapNotNull { pattern ->
          pattern.find(rawLine)?.groupValues?.getOrNull(1)?.toDoubleOrNull()
        }
        .firstOrNull()
  }
  ...
  private fun matchesDimension(normalized: String, descriptor: DimensionDescriptor): Boolean {
    ...
    if (
        Regex("""\bdim(?:ension)?\s*[:=#-]?\s*${Regex.escape(id)}\b""").containsMatchIn(normalized)
    ) {
      return true
    }
